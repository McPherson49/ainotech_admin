<div class="section-menu-left">
                    <div class="box-logo">
                        <a href="dashboard.php" id="site-logo-inner">
                            <img alt="" src="assets/images/logo.png" data-light="assets/images/logo.png" data-dark="assets/images/logo.png" >
                        </a>
                        <div class="button-show-hide">
                            <i class="icon-menu-left"></i>
                        </div>
                    </div>
                    <div class="center">
                        <div class="center-item">
                            <div class="center-heading">Main Home</div>
                            <ul class="menu-list">
                                <li class="menu-item has-children active">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-grid"></i></div>
                                        <div class="text">Dashboard</div>
                                    </a>
                                    <ul class="sub-menu" style="display: block;">
                                        <li class="sub-menu-item">
                                            <a href="dashboard.php" class="">
                                                <div class="text">Aino Dashboard</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="vendor_dashboard.php" class="active">
                                                <div class="text">Vendor Dashboard</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
                        <div class="center-item">
                            <div class="center-heading">All page</div>
                            <ul class="menu-list">
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-user"></i></div>
                                        <div class="text">Aino Tech User</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="user.php" class="">
                                                <div class="text">All user</div>
                                            </a>
                                        </li>                                    
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-user-plus"></i></div>
                                        <div class="text">Aino Tech Admin</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="admin.php" class="">
                                                <div class="text">All Aino Tech Admin</div>
                                            </a>
                                        </li>                                    
                                    </ul>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="add_admin.php" class="">
                                                <div class="text">Create Aino Tech Admin</div>
                                            </a>
                                        </li>                                    
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-layers"></i></div>
                                        <div class="text">Category</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="category.php" class="">
                                                <div class="text">Category list</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_category.php" class="">
                                                <div class="text">New category</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-layers"></i></div>
                                        <div class="text">Sub Category</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="sub_category.php" class="">
                                                <div class="text">Sub Category list</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_sub_category.php" class="">
                                                <div class="text">New Sub Category</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-user-plus"></i></div>
                                        <div class="text">Roles</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="role.php" class="">
                                                <div class="text">All roles</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_role.php" class="">
                                                <div class="text">Create role</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-box"></i></div>
                                        <div class="text">Discount Code</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="discount.php" class="">
                                                <div class="text">All Discount Code</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_discount.php" class="">
                                                <div class="text">Create Discount Code</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-box"></i></div>
                                        <div class="text">Promo Code</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="promo.php" class="">
                                                <div class="text">All Promo Code</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_promo.php" class="">
                                                <div class="text">Create Promo Code</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-box"></i></div>
                                        <div class="text">Voucher Code</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="voucher.php" class="">
                                                <div class="text">All Voucher Code</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_voucher.php" class="">
                                                <div class="text">Create Voucher Code</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-home"></i></div>
                                        <div class="text">Pick Up Station </div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="pick_up.php" class="">
                                                <div class="text">All Pick Up Station </div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add_pick_up.php" class="">
                                                <div class="text">Create Pick Up Station</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>


                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-shopping-cart"></i></div>
                                        <div class="text">Inventory</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="product-list.php" class="">
                                                <div class="text">Inventory List</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="add-product.php" class="">
                                                <div class="text">Add Product</div>
                                            </a>
                                        </li>
                                       
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-file-plus"></i></div>
                                        <div class="text">Order</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="oder-list.php" class="">
                                                <div class="text">Order list</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="oder-detail.php" class="">
                                                <div class="text">Order detail</div>
                                            </a>
                                        </li>
                                        <li class="sub-menu-item">
                                            <a href="oder-tracking.php" class="">
                                                <div class="text">Order tracking</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item has-children">
                                    <a href="javascript:void(0);" class="menu-item-button">
                                        <div class="icon"><i class="icon-file-plus"></i></div>
                                        <div class="text">Return Order</div>
                                    </a>
                                    <ul class="sub-menu">
                                        <li class="sub-menu-item">
                                            <a href="oder-list.php" class="">
                                                <div class="text">Order list</div>
                                            </a>
                                        </li>
                                    </ul>
                                </li>
                                <li class="menu-item">
                                    <a href="javascript:void(0);" class="">
                                        <div class="icon"><i class="icon-log-out"></i></div>
                                        <div class="text">Log out</div>
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>